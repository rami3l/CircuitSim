...
cktsim = (function () {
	...
	Circuit.prototype.find_solution = function (load, maxiters) {
		...
		for (var iter = 0; iter < maxiters; iter++) {
			...
			// Check residue convergence, but loosely
			// Lack a good comparison.
			converged = true;
			if ((iter == 0) || (abssum_rhs > abssum_compare)) {
				abssum_compare = abssum_rhs;
				converged = false;
			} else if (abssum_rhs >
				(res_check_abs + res_check_rel * abssum_compare)) {
				converged = false;
			}
	...
	function parse_source(v) {
		...
		// square(v_init,v_plateau,freq)
			...
			pwl_source(src, [0, v1, t_change, v2, t_change + t_pw,
				v2, t_change + t_pw + t_change, v1, per, v1
			], true);
		}
	...
	VSource.prototype.load_linear = function (ckt) {
		// MNA  for independent voltage source
